...
function version482#InitVersionDir(dirname)
    if ! has_key(s:versionDir, a:dirname)
        ...
        if v:shell_error || stridx(l:out, 'eecs482') < 0 || stridx(l:out, 'version482') >= 0
            let s:versionDir[a:dirname] = ''
        else
            " get project
            let l:matches = matchlist(l:out, 'eecs482/\([a-z.]*\)\.\(\d\)')
            if empty(l:matches)
		let s:versionDir[a:dirname] = ''
	    else
                " echo "matches=" . l:matches[1] . "," . l:matches[2]
		...
		if v:shell_error
		    " git rev-parse fails if not in a work tree
		    let s:versionDir[a:dirname] = ''
		else
			...
			if ! isdirectory(s:versionDir[a:dirname])
		        ...
		        " initialize Project 3-4 .version482 submodule
			if ! filereadable(fnamemodify(s:versionDir[a:dirname], ':p') . '.git')
			    call system('cd "' . l:top . '" ; git submodule update --init ')
			endif
			if ! filereadable(fnamemodify(s:versionDir[a:dirname], ':p') . '.git')
			    " couldn't initialize submodule
			    let s:versionDir[a:dirname] = ''
			else
			    ...
			    if l:branch1 != l:branch
			        ...
			        " create and switch to branch, and set up tracking
				" relationship between local repo and github
			        call system('cd "' . s:versionDir[a:dirname] . '"; git checkout -b ' . l:branch)
				if v:shell_error
				    " git checkout -b fails if the branch already exists
				    " try just checking out the branch
...
function version482#TextChanged(...)
	...
	" commit changes
	call system('cd "' . l:versionDirname . '"; git add "' . l:basename . '"; git commit --allow-empty -m "'. s:version . '"')
